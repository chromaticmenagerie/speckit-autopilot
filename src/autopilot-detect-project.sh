...
for arg in "$@"; do
    case "$arg" in
        ...
        --patch) PATCH=true ;;
        -h|--help)
            echo "Usage: autopilot-detect-project.sh [--force|--patch]"
            echo "  Detects project tooling and writes .specify/project.env"
            echo "  --force   Overwrite existing project.env"
            echo "  --patch   Append missing variables to existing project.env"
            exit 0
            ;;
    esac
done

if [[ "$FORCE" == true ]] && [[ "$PATCH" == true ]]; then
    echo "Error: --force and --patch are mutually exclusive"
    exit 1
fi

if [[ -f "$ENV_FILE" ]] && [[ "$FORCE" != true ]] && [[ "$PATCH" != true ]]; then
    echo "project.env already exists at $ENV_FILE"
    echo "Use --force to overwrite or --patch to add missing variables."
    exit 0
fi
...
if [[ "$PATCH" == true ]]; then
    if [[ ! -f "$ENV_FILE" ]]; then
        echo "Warning: no existing project.env — falling back to fresh install"
        _render_env_template "$ENV_FILE"
    else
        _patch_tmp=$(mktemp)
        trap "rm -f '$_patch_tmp'" EXIT
        _render_env_template "$_patch_tmp"
        _patch_env_file "$ENV_FILE" "$_patch_tmp"
        rm -f "$_patch_tmp"
    fi
else
    _render_env_template "$ENV_FILE"
